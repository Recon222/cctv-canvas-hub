...
export class ErrorBoundary extends Component<Props, State> {
  ...
  override componentDidCatch(error: Error, errorInfo: ErrorInfo) {
    ...
    this.setState({ errorInfo, saveStatus: 'saving' })

    // Save crash state asynchronously (don't block error UI)
    void this.saveCrashData(error, errorInfo)
  }
  ...
  private async saveCrashData(error: Error, errorInfo: ErrorInfo) {
    try {
      ...
      // Don't throw from error boundary - log and reflect the failure in the UI
      // so we never tell the user their data was saved when it wasn't.
              ...
              <p className="text-muted-foreground mb-6">
                {this.state.saveStatus === 'saved'
                  ...
                  : this.state.saveStatus === 'failed'
                    ? 'The application encountered an unexpected error. We could not save your data automatically — recent changes may be lost.'
                    : 'The application encountered an unexpected error. Attempting to save your data…'}
              </p>
            </div>
