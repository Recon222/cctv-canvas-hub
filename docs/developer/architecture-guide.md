...
```
App (Top-level orchestrator — bootstraps systems, no routing)
└── ThemeProvider
    └── MainWindow (Composes layout with features)
        ...
        └── Global Overlays
            ├── PreferencesDialog (from features/preferences)
            ├── CommandPalette (from features/command-palette)
            └── Toaster (Notifications)
```

Data flow: `App → ThemeProvider → MainWindow → Features`
...
src/
├── features/            # Domain features (primary organization)
│   ├── <feature>/
│   │   ├── components/  # UI components
│   │   ├── hooks/       # TanStack Query hooks (orchestration layer)
│   │   ├── services/    # Plain async functions wrapping Tauri IPC
│   │   ├── store/       # Feature-local UI-only state (Zustand, if needed)
│   │   ├── types/       # Feature types
│   │   ├── __tests__/   # Feature tests
│   │   └── index.ts     # Public API (barrel export)
│   ├── preferences/     # User preferences feature
│   ├── command-palette/ # Command palette feature
│   ├── quick-pane/      # Quick pane floating window feature
...
│   ├── services/        # Cross-cutting services (events)
│   ├── types/           # Shared TypeScript types
│   └── menu.ts          # Native menu builder with i18n
├── store/               # Global UI-only Zustand stores
└── test/                # Test utilities and setup
```

### Feature Architecture Rules

1. **Barrel exports**: Outside code imports from `@/features/<name>`, never internal paths
2. **Service layer owns IPC**: All `commands.*` calls live in `services/` as plain async functions
3. **Hooks orchestrate**: TanStack Query wraps service functions for caching/loading states
4. **Features are self-contained**: Each feature owns its components, hooks, services, and types
5. **Cross-feature communication**: Use events (`lib/services/featureEvents.ts`), not direct imports
...
1. **Create feature directory** - Copy `src/features/example-feature/` as a starting point
2. **Services** - Plain async functions in `services/` wrapping `commands.*` from tauri-bindings
3. **Hooks** - TanStack Query hooks in `hooks/` wrapping service functions
4. **Components** - UI components in `components/` using hooks for data
5. **Barrel export** - Export public API from `index.ts`
6. **Commands** - Add feature commands to `lib/commands/feature-commands.ts`
7. **State** - Choose appropriate layer (useState/Zustand/TanStack Query)
8. **Testing** - Write tests for hooks and services (see [testing.md](./testing.md))
9. **i18n** - Add translation strings to `locales/*.json`
