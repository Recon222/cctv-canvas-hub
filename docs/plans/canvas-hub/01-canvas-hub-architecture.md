...
4. Safety net for lost broadcasts: case-data queries also refetch on a slow interval (**`RECONCILE_MS`, 35 s — PR #8 H1 two-cycle margin; was 60 s (A2), itself lowered from 5 min after the PR #4 cadence finding**). The reconcile is the **only positive liveness confirmation on a silent agency**, so TWO full cycles plus the jitter budget must fit inside the stale threshold — one missed or slow cycle must never flash a false STALE: `2 × RECONCILE_MS + FETCH_BUDGET_MS ≤ STALE_AFTER_MS` is an invariant with a test tripwire. Broadcast is best-effort with no replay — a silently dropped event on a healthy socket must not outlive one reconciliation cycle (spec §6: never present stale as live).
