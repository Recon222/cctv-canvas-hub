...
| #   | Test Description                                                          | Key Assertion                                                                 |
| --- | -------------------------------------------------------------------------- | ------------------------------------------------------------------------------- |
| 122 | Should render canvass activity in the ACTIVITY lane                        | activity-ring entries appear newest-first through the source adapter — the lane hosts `ActivityFeed` (reuse), fed via the canvass barrel's read-only selector (6.3E) |
...
| 125 | Should default the panel posture per view (AD14 revised)                   | fresh mount on `cases`/`case` ⇒ panel expanded, ACTIVITY lane active (wall posture — spec §6 attention surface visible); on the `map` view ⇒ collapsed to the SYS tab (expanded-on-map overlays the card stack — map and stack never reflow) |
...
| 127 | Should tail cleanly across a mid-codepoint slice boundary (Rust)           | `tail_log` on a slice starting mid-UTF-8-codepoint never errors — `from_utf8_lossy` + first-partial-line drop yields only clean lines |
| 128 | Should clamp to the last `max_lines` lines (Rust)                          | input above the clamp returns exactly the newest `max_lines` lines             |
| 129 | Should return the whole input when shorter than the window (Rust)          | short input ⇒ all lines intact, first line included (the slice is not mid-file — nothing dropped) |
