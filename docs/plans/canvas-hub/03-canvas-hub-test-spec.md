...
| #   | Test Description                                                          | Key Assertion                                                                 |
| --- | -------------------------------------------------------------------------- | ------------------------------------------------------------------------------- |
| 138 | Should push session-token on TOKEN_REFRESHED and on nothing else           | `initSupabase`'s `onAuthStateChange` wiring emits `{url, key, token}` on TOKEN_REFRESHED; SIGNED_IN / SIGNED_OUT push nothing — and NO eager revocation detection is wired (ledger D19 stays accepted; this row is its named reconsider-point, consciously passed) |
