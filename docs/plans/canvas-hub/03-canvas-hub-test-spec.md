...
No existing test file is deleted or rewritten, with three sanctioned exceptions: **#98 is amended in place at Phase 6.3C** (the feed relocates to the panel — the test flips to assert the recomposed dashboard), **`LocationCard.test.tsx`'s selection-a11y test is amended in place at Phase 3.4A** (ledger D16 migrates the card from `role="button"` to the single-select `role="option"`/`aria-selected` model — the old role query is the exact thing D16 changes), and **#135 is amended in place at M7** (ledger L3: `cameraPadding` now takes the measured board width — the call sites change, the pinned RTL-mirror behavior does not; an unnumbered width-scaling rider lands beside it). **As-built 6.3C riders on the #98 relocation (M6):** #90's scoping arm retargeted its subject in place (`CaseDashboard` → `PanelActivityLane`, the code that owns the case filter after the relocation — the pinned behavior is unchanged), and two `casesView.test.tsx` header-chrome arms scoped their state-word queries to the header banner (the panel footer now renders the same word; the pins stay on the D15 chip). Seven existing files receive **additions** only (`src/lib/commands/commands.test.ts`, `src-tauri/crates/platform-utils/src/lib.rs`, `src/features/canvass/__tests__/realtime.test.ts` — R6, `src/features/canvass/__tests__/LocationCard.test.tsx` — #88 at 4.3, M4, and — R8/M7 — `src/lib/supabase/client.test.ts` (#138–139), `src/features/cloud-session/__tests__/authService.test.ts` + `src/features/cloud-session/__tests__/session-store.test.ts` (the two #140 halves)). Before Phase 1.4 removes the sidebar panels, audit existing component/hook tests for assertions pinned to that layout and re-home them in the same commit — nothing pinned may silently disappear.
...
## Revision R8 additions (M7 implementation — main-side session propagation)

#138–140 belong to **Phase 7.2** (counted there in the summary). Written during M7: the pre-numbered #114–117 pin the SECONDARY side of AD13; nothing pinned what the main window must emit. #138–139 are additions to `src/lib/supabase/client.test.ts`; #140 is split across `authService.test.ts` (sign-out half) and `session-store.test.ts` (lock half). An unnumbered teardown rider on #117 (the client half — channels removed, realtime disconnected, holder released, auth.* never touched) rides in `secondary-client.test.ts`.

| #   | Test Description                                                          | Key Assertion                                                                 |
| --- | -------------------------------------------------------------------------- | ------------------------------------------------------------------------------- |
| 138 | Should push session-token on the full session-rotation class **(flipped by the PR #10 review, H1)** | `initSupabase`'s `onAuthStateChange` wiring emits `{url, key, token}` on TOKEN_REFRESHED, SIGNED_IN (the M6 unlock re-auth mints a new session via `signInWithPassword` — the original "SIGNED_IN pushes nothing" pin locked in the unlock-orphan defect), and USER_UPDATED; SIGNED_OUT pushes nothing — NO eager revocation detection is wired (ledger D19 stays accepted). An unnumbered unlock-path rider (real `signInWithPassword` against a stubbed fetch ⇒ the minted token is pushed) rides in the same file |
| 139 | Should reply to secondary-ready with the current token only when signed in | the handshake reply carries the LIVE access token (+ designed-public url/key); a signed-out main stays silent — the secondary hits its boot timeout honestly, never receives a dead token |
| 140 | Should broadcast lock parity and sign-out termination from their choke points | `signOut()` emits `session-ended` exactly once (sign-out ONLY — a lock never ends secondaries); `lock()`/`unlock()` emit `session-locked`/`session-unlocked` from the same choke point that persists the durable flag; guarded no-op transitions emit nothing |
...
**Total: 140** (Rust 9 — 6 `secure-vault` + 3 `platform-utils` · TypeScript 131; #107 → R1/2.2, #108 → R2/6.1, #109 → R3/1.2, #110–121 → R4/A1 with #120 reassigned to 6.3 by A2, #122–129 → R5/A2 with #127–129 the Phase 6.3B′ Rust tests, #130–131 → R6/2.3 (ledger D17, added at M3), #132–135 → R7/3.3 and #136–137 → R7/3.2 (PR #6 review M3 + L4 + M4), #138–140 → R8/7.2 (M7 implementation — main-side session propagation); ported 6.3A test files are excluded — this count is canvas-hub-authored tests only) — reconciles with the Implementation Plan, Appendix C. **Rule:** if counts drift during implementation, reconcile both documents before proceeding to the next phase.
