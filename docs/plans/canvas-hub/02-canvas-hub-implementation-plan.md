...
| AD9 (superseded in part by A2) | Template sidebars (OD9)   | Stop rendering panels in `MainWindow`; files stay  | Smallest diff that satisfies "no edge-docked panels". _Deleting files rejected for V1_: touches menus/shortcuts/commands/tests for zero product value; scheduled for a later `/cleanup`. **Supersession note (A2 fix round):** AD14 revised refills the right-edge chrome position this decision emptied — spec §4's "no edge-docked panels" wording is **owner-superseded for that one chrome surface** (the collapsible ProcessPanel; on the map view it auto-collapses to a SYS tab and, when expanded, overlays — it never reflows the map). The sidebar FILES stay unrendered; the panel is a bespoke board-tree component (6.3C) that reuses `rightSidebarVisible` as its expanded state. |
...
| AD11 | Cross-feature seams (net-new; no OD) | Connection health lives in the **global layer** (`src/store/health-store.ts` + `src/hooks/useConnectionHealth.ts`), not inside either feature — `HealthState` and `ChannelStatus` are defined there, once; catch-up invalidation targets **case-data queries only** via an **allow-list** (as-built M2 — the fix-delta round inverted the originally planned deny-list): `CASE_DATA_KEY_FAMILIES` in the health-store is the **single registry** of case-data key families, and the predicate is `isCaseDataKey(query.queryKey[0])` (`useConnectionHealth`). **Registration obligation: every new case-data query family (M3/M5 and beyond) MUST be registered in `CASE_DATA_KEY_FAMILIES`, or it gets no catch-up refetch** — after a sleep/wake it would render pre-sleep data behind a green `live` indicator, the exact G4 violation this plan exists to prevent. Signed URLs are excluded *by not being in the allow-list* — they refresh on their own interval (4.1B) and must not be mass-regenerated on every wifi blip; the only feature→feature imports are **one-way, read-only barrel consumptions**: canvass → cloud-session (session state for gating/lock state), canvass/cloud-session → preferences via the **barrel-exported `usePreferences` hook**, and (A2 fix-delta 2) **no process-panel → canvass import**: the panel's ACTIVITY lane is filled by composition (`CanvassRoot` injects `<ActivityFeed />` via the panel's `activitySlot` prop — the inventory above stays exhaustive) — the preferences *service* is not barrel-exported, and `App.tsx`'s existing relative deep import of it predates this plan and must not be copied (an aliased deep import fails `ast:lint`). `lib/supabase/vault-storage.ts` calling `commands.*` outside a feature `services/` dir is a sanctioned adapter (it *is* the storage seam of the client singleton). | Architecture-guide rule 5 says features communicate via events, and today zero feature→feature imports exist — so the seams are named and bounded here instead of appearing ad hoc. _Event-bus health signals rejected_: health is shared **state** (a store), not a notification; the global-store layer is the documented home for exactly this. _cloud-session-owned health rejected_: canvass would then import cloud-session services (a cycle risk). _Unfiltered invalidation rejected_: it refetches every mounted signed-URL query — N storage calls + a visible thumbnail flash per reconnect. |
...
| AD14 (**revised A2; port scope pinned A2 fix round**) | Process panel, not a diagnostics window | **Right-side collapsible ProcessPanel** in the right-edge chrome position, mounted in the board tree (`CanvassRoot`'s flex row — 6.3C; there is no "RightSideBar slot": neither template sidebar renders anywhere), toggling **ACTIVITY** (hosts `ActivityFeed.tsx` — relocated out of the dashboard column at 6.3; the dashboard hosts the feed as interim through M5) ↔ **SYSTEM** (health transitions, source-tagged log tail via `read_log_tail`, `vault_status`, uptime/versions). Collapsed = slim SYS tab. **Default-open on ACTIVITY on the `cases`/`case` views** (wall posture — the attention surface stays visible, spec §6); **on the `map` view the panel auto-collapses to the SYS tab**, and if expanded there it **overlays** the floating card stack — map and stack never reflow (no width coupling, no resize trigger). Expanded/collapsed state **is `useUIStore.rightSidebarVisible`** (default flipped open); only lane selection is new panel-local state. Implementation is a **gate-passed adaptation, not a copy**, of the retained surface of Kris's `processTerminal` feature (`D:\Coding\Agents_and_Skills\timeline-agent-sdk\claude-sdk-timeline-creator\src\features\processTerminal` — same stack: React/Zustand/i18next/vitest, no terminal lib): the pure lane renderers + supporting services come across behind the `sources/canvasHubSource.tsx` adapter; the agent-pipeline contract (`ProcessTerminal` dispatcher, `SessionPanel`, sources) is discarded — retained/discarded sets and the adapter signature are pinned in 6.3A. **No new dependency**: `TableCard` (the `react-data-grid` consumer) is in the discarded set — the dep decision is made HERE, not at port time: dropped | _A1's secondary window rejected (owner, 2026-07-20)_: the panel completes the sidebar symmetry, shrinks M7, removes secondary-auth complexity for diagnostics, and collapsibility answers the wall-real-estate objection that motivated the window. _Terminal-emulator lib rejected_: the ported component is a bespoke renderer. _"Port as shipped" rejected (A2 fix round)_: the source's render boundary is agent-pipeline-shaped (`SessionData`/`PipelineMessage`) and four of its `@/features/*` aliases don't exist in this repo — a straight copy fails `tsc` on the first file. |
| AD15 (A2) | Scale-to-fit shell strategy | The design canvas is 1920×1080 with `transform: scale(vw/1920)` on the shell. As-built: **scale the chrome only; the Mapbox canvas stays unscaled-native** (map fills its container at real pixels; chrome overlays scale) — final mechanics decided at Phase 3.2 with a live check. **The transform's owning file is a named phase row: `CanvassRoot.tsx` (3.2E, fix-delta 2)** — the only node whose children split into "hoisted map div" and "chrome"; the chrome children carry the scale, the map sibling stays outside every scaled ancestor (no `MainWindow`-level wrapper can exclude the map — the DOM chain puts it inside anything that file owns) | _Whole-shell scaling rejected pending proof_: Mapbox GL inside a CSS-transformed ancestor has known pointer-math and tile-crispness pitfalls; if the M3 live check shows the simple whole-shell scale works flawlessly on the target display, the simpler option may win — the decision point is pinned, not the outcome. |
...
| **M5**    | Attention & dashboard: feed UI, pulses, connection indicator, dashboard view | Feed shows most-recent-first case-scoped entries; markers/cards pulse on change; pulling the network shows reconnecting → STALE honestly; dashboard shows counts/roster/feed (the feed's **interim** home — 6.3 relocates it into the ProcessPanel's ACTIVITY lane). |
...
| 2.2B | `src/features/canvass/hooks/{useCases,useCaseLocations,useCaseMedia,useLocationCounts}.ts` | NEW | query keys built from the health-store family constants (`CASES_KEY`/`LOCATIONS_KEY`/`MEDIA_KEY`/`LOCATION_COUNTS_KEY` — A2 as-built, never bare strings): `['cases']`, `['locations', caseId]`, `['media', caseId]`, `['location-counts']` (the landing counts query the §5.2 pre-filters invalidate); success paths call `recordFetchOk()`, failures `recordFetchError()` (the `live` predicate needs positive confirmations). Cases/locations also get `refetchInterval: RECONCILE_MS` (**35 s — PR #8 H1 two-cycle margin; was 60 s A2**; invariant `2 × RECONCILE_MS + FETCH_BUDGET_MS ≤ STALE_AFTER_MS`, test-pinned) — the lost-broadcast safety net (Flow E4). **Build-order note:** depends on the health-store (2.5A) — within M2, land 2.5A first; phase numbering is narrative grouping, not build order |
...
| ID   | File                                                | Tag | Signatures                                                                                                                            |
| ---- | --------------------------------------------------- | --- | -------------------------------------------------------------------------------------------------------------------------------------- |
| 2.3A | `src/features/canvass/services/realtimeService.ts`  | NEW | the doc 01 §5.2 contract — **authoritative there, never restated here** (as-built A2: the five-arg mount-scoped thunk form `subscribeToCaseActivity(getCaseId, onEvent, onStatus, onLocationTraffic?, onCaseTraffic?): () => void` — the case id is read through the getter at delivery time; the channel is never case-keyed) |
...
| ID   | File                             | Tag | Signatures                                                                                                                       |
| ---- | -------------------------------- | --- | --------------------------------------------------------------------------------------------------------------------------------- |
| 2.5A | `src/store/health-store.ts`      | NEW | `useHealthStore` (devtools, selector-only) · actions `recordEvent()` · `recordFetchOk()` · `recordFetchError()` · `channelStatus(s: ChannelStatus)` · exported pure `evaluate(marks, now): HealthState` · **canonical type definitions**: `HealthState = 'connecting' \| 'live' \| 'reconnecting' \| 'stale' \| 'offline'` and `ChannelStatus` (mapped from supabase-js subscribe states) — every other module imports these, never re-declares · constants `STALE_AFTER_MS = 90_000`, `RECONCILE_MS = 35_000`, `FETCH_BUDGET_MS = 20_000` (**PR #8 H1 — two-cycle margin; was 60 s A2**, itself lowered from 5 min by the PR #4 cadence finding; **invariant: `2 × RECONCILE_MS + FETCH_BUDGET_MS ≤ STALE_AFTER_MS`, test-pinned** by the health-store tripwire), `SIGNED_URL_KEY_PREFIX = 'signed-url'` · the case-data family constants (`CASES_KEY`/`LOCATIONS_KEY`/`LOCATION_COUNTS_KEY`/`MEDIA_KEY`), the `CASE_DATA_KEY_FAMILIES` tuple, and `isCaseDataKey()` — the catch-up **allow-list** registry (AD11): every case-data family registers here or gets no catch-up |
| 2.5B | `src/hooks/useConnectionHealth.ts` | NEW | wires listeners (`online`/`offline`/`visibilitychange`) + interval `evaluate` → health-store; reconnect ⇒ invalidate **case-data queries** via the **allow-list predicate** `isCaseDataKey(query.queryKey[0])` over `CASE_DATA_KEY_FAMILIES` (imported from health-store — same milestone, no forward reference). Signed URLs are excluded by *not being in the allow-list*; **every new case-data query family must be registered in `CASE_DATA_KEY_FAMILIES` or it gets no catch-up refetch** (Flow E, AD11) |
...
| 3.1C (A2) | `src/assets/fonts/` + Tailwind v4 theme (`src/theme-variables.css` `@theme` — the template's token home, imported by both `App.css` and `quick-pane.css`, so tokens reach the quick-pane entry and M7's `window.html`; `App.css` itself is three imports) | NEW/MODIFY | vendor the Case File design system: woff2 fonts (Nacelle, Inter, JetBrains Mono, Share Tech Mono) + the handoff's color/status tokens as CSS variables — the binding design language for all M3–M6 UI (`design_handoff_canvas_hub/README.md`) |
...
| 3.2B | `src/features/canvass/components/MapCanvas.tsx`     | NEW    | `react-map-gl/mapbox` `<Map>`; viewport from incident coord; `MapTokenGate` inner component when token absent (designed state: grid-paper ground, board-still-works copy); token-rejected ⇒ toast. **A2 bindings:** default style satellite + `setConfigProperty('basemap','lightPreset','night')`. Map-div persistence and `resize()` ownership live in **3.2D** (`CanvassRoot` — a child cannot prevent its own unmount when the parent's view conditional flips); the AD15 scale strategy lives in **3.2E** (`CanvassRoot` — fix-delta 2), live-checked during this phase |
| 3.2C | `package.json`                                      | MODIFY | add `mapbox-gl` + `react-map-gl` (supabase-js landed in 1.2H)                                                                          |
| 3.2D (A2 fix round) | `src/features/canvass/components/CanvassRoot.tsx` | MODIFY | hoist `MapCanvas` **above the view switch** — mounted once for the life of the board, hidden when `view ≠ 'map'` by a **laid-out-but-invisible mechanism** (`visibility: hidden` or off-screen positioning: the container must keep real dimensions; **never `display: none`** — a later `map.resize()` would measure 0×0 and collapse the canvas). `map.resize()` trigger set, exhaustive: **switch to the `map` view + window resize** — the ProcessPanel overlays and never resizes the map (AD14) |
| 3.2E (fix-delta 2) | `src/features/canvass/components/CanvassRoot.tsx` | MODIFY | carries the **AD15 shell transform**: `transform: scale(vw/1920)` on CanvassRoot's chrome children only — the hoisted map div is their sibling, outside every scaled ancestor (unscaled-native); final mechanics live-checked here per AD15 (whole-shell fallback stays available if the live check prefers it — the decision point is pinned, not the outcome) |
...
| ID   | File                                                    | Tag    | Signatures                                                              |
| ---- | ------------------------------------------------------- | ------ | ------------------------------------------------------------------------ |
| 3.4A | `LocationCardStack.tsx` + `LocationCard.tsx` + `CanvassRoot.tsx` | MODIFY | overlay positioning (floating, scrollable, no full-height rails); large-type styles. **Resolve ledger D16 here** (single-select a11y): `role="listbox"` on the stack + `role="option"`/`aria-selected` on each card — one DOM subtree owns the whole model (options must be owned by their listbox; splitting roles across the WebGL marker layer and the stack is invalid ARIA and AT reports nothing) |
...
| 4.3C (fix-delta 2; namespace amended M4) | `locales/{en,fr,ar}.json` | MODIFY | `canvass.viewer.*` keys (as-poured namespace — the design pour shipped the viewer keys under `canvass.viewer.*`; this row previously said `canvass.imageViewer.*`) — user-facing wall copy ships with its component's milestone, not two milestones later |
...
| ID   | File                                                   | Tag | Signatures                                                                                     |
| ---- | ------------------------------------------------------ | --- | ----------------------------------------------------------------------------------------------- |
| 5.1A | `src/features/canvass/components/ActivityFeed.tsx`     | NEW | most-recent-first, case-scoped; timestamps per doc 01 rule 6 — **absolute with seconds** (dates explicit `yyyy-mm-dd`), a relative age only as a secondary annotation, never a replacement. **M5 interim host: the dashboard's feed column (5.3A)**; Phase 6.3C relocates this same component into the ProcessPanel's ACTIVITY lane (component reuse — the lane hosts `ActivityFeed`, not the ported row renderer) |
...
| ID   | File                                                 | Tag    | Signatures                                                                                              |
| ---- | ---------------------------------------------------- | ------ | -------------------------------------------------------------------------------------------------------- |
| 5.3A | `src/features/canvass/components/DashboardView.tsx`  | NEW    | per the locked design (A2): incident panel + four stat tiles + media strip + **investigator roster** grid (AD8; must handle 15+ investigators; location rows expand inline to the same detail block the map cards use — DVR grid, requested-video windows, notes, media thumbs) + the **activity feed** (5.1A) in the right column — **interim until 6.3** (every milestone ships a working app; the feed must render somewhere at M5, and a zero-call-site `ActivityFeed.tsx` would fail knip). Phase 6.3C moves the feed into the ProcessPanel's ACTIVITY lane and recomposes this view — the roster takes the freed column (test #98 updated then) |
...
### Phase 6.3 — ProcessPanel: the adapted terminal + ACTIVITY ↔ SYSTEM toggle (A2) ⚠

**Goal:** AD14 (revised) becomes real — the right-side collapsible panel in the board tree, fed by canvas-hub sources. The port is a **gate-passed adaptation, not a copy**: the source repo's four `@/features/*` aliases (`shared`, `agentPipeline`, `clawDirect`, `renderers`) do not exist in this repo — every retained file is re-pointed to a canvas-hub home, or its importer is in the discarded set.

| ID   | File                                                       | Tag    | Signatures                                                                                                     |
| ---- | ----------------------------------------------------------- | ------ | ------------------------------------------------------------------------------------------------------------------ |
| 6.3A | `src/features/process-panel/` (adapted `processTerminal` surface + adapter) | NEW | **Retained surface (fix-delta 2 cut — verified to compile in isolation):** `TextLane.tsx`, `ansiParser.tsx`/`ansiSgr.ts`/`parseAnsiToHtml.ts`/`vtEngine.ts`, `crt.css`, and `Header.tsx`/`Footer.tsx` chrome with two adaptations: `ProcessTerminalBgMode`/`bgMode` **cut** (the adaptation hardcodes the CRT treatment), and Footer's `PipelineResult` token-count block **cut** (drops `formatTokenCount` with it). **As-built narrowing (M6 live smoke):** `parseAnsiToHtml.ts` left the retained set — the plan pinned it from the source repo's HTML-export usage, which has no canvas-hub consumer (the lane displays via `vtEngine`/`TextLane`); its one wired call site, a SYSTEM-lane export, was cut rather than granting the kiosk fs-write capability, and `ansiSgr.ts`'s `cssStringFrom*` helpers left with it. This set's only external imports are `@/lib/utils`, `@/components/ui/{button,dropdown-menu}`, `react-i18next`, `lucide-react` — all present here — and it consumes exactly `ProcessPanelRow`. **Discarded:** the `ProcessTerminal` dispatcher, `SessionPanel` and its `SessionData`/`PipelineMessage` contract, `sources/ProcessTerminal{SDK,Claw}.tsx`, the agent-pipeline feed model (`feedReducer`/`coalescer`/`displayData`/`displayPayloads`), **`RichLane.tsx` + all `rich/*` cards** (their prop contract is `DisplayItem`, a discriminated union living in the discarded `displayPayloads` — structurally incompatible with the flat `ProcessPanelRow`; retaining them was the fix round's error), **`EmptyPanel.tsx`** (renders the source repo's changelog — no canvas-hub job), `TableCard` (+ `react-data-grid` — AD14: dropped, now with literally zero referents), and `BlockFeed`/`CallBlockView`/`NoteRowView`. **Adapter pinned as a signature:** `sources/canvasHubSource.tsx` producing `ProcessPanelRow { at: number; lane: 'activity' \| 'system'; source: string; text: string; tone?: 'info' \| 'warn' \| 'error' }`, fed by (a) a vanilla `useHealthStore.subscribe` diff (~5 lines — the store keeps **no transition log** and is **not** created with `subscribeWithSelector`, so the adapter diffs **`state.state`** across notifications — diffing the store object would emit a row per envelope and bury real transitions), (b) `readLogTail` polling via a diagnostics service wrapping `commands.readLogTail`/`commands.vaultStatus`, and (c) **no canvass import at all**: the ACTIVITY lane renders an `activitySlot: ReactNode` prop that `CanvassRoot` (a canvass file) fills with `<ActivityFeed />` — composition at the mount site, no cross-feature seam (fix-delta 2). **Poll discipline:** `LOG_TAIL_POLL_MS = 5_000`; the poll runs only while *expanded AND the SYSTEM lane is active* (a kiosk session is measured in days — no 64 KB disk reads for a lane nobody is looking at); a tick is skipped while a read is in flight (an unguarded interval stacks concurrent reads). The ACTIVITY lane **hosts `ActivityFeed.tsx` via the `activitySlot` prop** (component reuse, relocated from the dashboard by 6.3C — not the ported row renderer). **Ported tests:** only files covering the retained surface come across (`ansiParser`, `vtEngine`/`TextLane`), adapted to compile; dispatcher/source tests are dropped with their subjects. Ported tests run in the gate but are **excluded from the numbered count** — doc 03 counts canvas-hub-authored tests only. Gate-passed: strictTypeChecked, ast-grep, i18n keys (6.3D), selector-only stores |
| 6.3B | `src-tauri/src/features/cloud_session/{commands,services,types}/mod.rs` + `bindings.rs` | MODIFY | `read_log_tail(app, lines: u32) -> Result<String, String>` — clamp ≤ 500 lines, seek-from-end ≤ 64 KB, path = `app_log_dir()` joined with a filename **derived from `app.package_info().name`** (never a literal: a `productName` rename must not silently strand the SYSTEM lane on a permanently empty file), defensively redacted; the tail slicing itself is the pure `platform-utils::tail_log` (6.3B′) — the command reads the file and delegates. `vault_status(app) -> Result<VaultStatus, String>` — a locked or unreachable keychain is an **error**, never "absent" (an `Ok` of all-false would read as "no key present" and send the operator to re-enroll); `VaultStatus` is a new four-derive IPC type in `types/mod.rs` with fields `config_present` / `vault_present` / `keyring_key_present` + `vault_mtime_ms: Option<f64>` (f64 — specta rejects u64). **`vault_get` is the wrong tool for status** — it decrypts the session; status must never touch plaintext. Both commands move HERE from the old 7.1A (the panel is their consumer, not the windows). Widen the `cloud_session` module doc comment by one line to cover the log tail (discoverability). Regenerate bindings |
| 6.3B′ | `src-tauri/crates/platform-utils/src/lib.rs` | MODIFY | pure free function `pub fn tail_log(bytes: &[u8], max_lines: usize, partial_first_line: bool) -> String` — the caller (6.3B) passes `partial_first_line: true` **only when it actually seeked** (the function cannot know from bytes alone whether byte 0 is a real line start — fix-delta 2); `String::from_utf8_lossy` (a seek-from-end read can land mid-codepoint; the log is not ASCII-only — `TargetKind::Webview` routes frontend `fr`/`ar` strings into the same file), drop the first line only when `partial_first_line`, clamp to the last `max_lines`. Inline `#[cfg(test)]`: 3 tests (#127–129 — mid-codepoint split; clamp above the window; input shorter than the window). The Tauri-free workspace crate is the one place this logic is unit-testable on Windows (`[lib] test = false` in the app crate) |
| 6.3C | `CanvassRoot.tsx` + `DashboardView.tsx` + panel chrome | MODIFY | **Mount site: inside `CanvassRoot`'s flex row** (the board tree — gated on `active`/`locked` like everything there, so the SYSTEM poll and ACTIVITY lane never run during `booting`/`needs-setup`/`signed-out`/`schema-gate` on an unattended board, and the panel's lifecycle aligns with the ring/store resets on unmount). There is no "RightSideBar slot" — neither template sidebar renders anywhere (A1's NavRail precedent is likewise a bespoke board-tree component); the panel occupies the **right-edge chrome position** as owner-superseded chrome (AD9). **Expanded/collapsed state IS `useUIStore.rightSidebarVisible`** (the template default is already `true` — no store change, fix-delta 2) — the template's existing shortcut and show/hide-right-sidebar palette commands drive the panel with zero new plumbing; only **lane selection** is new panel-local state. **Posture:** default-open on ACTIVITY on the `cases`/`case` views (wall posture — spec §6); on the `map` view the panel **auto-collapses to the SYS tab**, and when expanded there it **overlays** the card stack — map and stack never reflow (no width coupling, no `map.resize()` trigger — 3.2D). **Precedence rule (fix-delta 2 — two writers, one boolean):** the view-derived posture applies on **first entry to each view per session** (a `viewsSeen` set); an explicit user toggle wins thereafter — a manual expand on the map view survives `map → case → map`, and the shipped shortcut never reads as flaky. Collapsed = slim SYS tab; header toggle per the design. **Feed relocation:** `ActivityFeed` moves out of `DashboardView` into the ACTIVITY lane; the dashboard recomposes — the roster takes the freed column (test #98 updated in place to assert the recomposition) |
| 6.3D | `locales/{en,fr,ar}.json` | MODIFY | `processPanel.*` keys (lane labels ACTIVITY/SYSTEM, the collapsed SYS tab, inline error rows — ported keys re-namespaced under `processPanel.*`; `ar.json` is RTL). (`canvass.viewer.*` moved to 4.3C — fix-delta 2: keys ship with their component's milestone; namespace as-poured, amended M4) |
| ~~6.3E~~ | — | — | **Removed (fix-delta 2)** — no barrel export and no selector: the panel never imports canvass. `CanvassRoot` injects `<ActivityFeed />` as the panel's `activitySlot` child (composition at the mount site); a selector would have zero call sites — the plan's own knip argument |
| 6.3F | `src/test/setup.ts` | MODIFY | mock `readLogTail` + `vaultStatus` (the 1.2G precedent — #120/#123 must mount the panel) |

**Error handling:** a failed log-tail read renders an inline panel error row, never breaks the board; a `vault_status` error renders as an explicit unknown/error row — never as "no key present". The panel is chrome — its failure modes stay inside it.
...
### Phase 7.3 — Pop-out surfaces (A1; diagnostics removed by A2)

**Goal:** the product part — Map and Case dashboard open in their own windows; Cases stays bound. (A2: there is no diagnostics window — the ProcessPanel owns that content, Phase 6.3.)

| ID   | File                                                     | Tag    | Signatures                                                                                                     |
| ---- | --------------------------------------------------------- | ------ | ------------------------------------------------------------------------------------------------------------------ |
| 7.3A | `src/features/canvass/components/SecondaryRoot.tsx`      | NEW    | hosts `MapCanvas`-view or `DashboardView` with its own QueryClient + realtime subscription. Boot order pinned: **attach `session-token`/`view-context`/`session-locked`/`session-ended` listeners FIRST, then** emit `secondary-ready` (else the handshake reply re-races) → token installed (`initSecondaryClient`, which claims this context's `getSupabase()` seam) → `view-context` seeds **this context's own** canvass-store (`selectCase(caseId)`, `setView`) → queries + the mount-scoped subscription mount — `subscribeToCaseActivity(getCaseId, …)` in the five-arg thunk form (doc 01 §5.2), where `getCaseId` reads **this context's** canvass-store (a positional case id is a type error, and would freeze the channel to one case — the getter exists precisely so a secondary reads its own store at delivery time). **Session-exit purge, per context (doc 01 §5.4 invariant):** on `session-ended` this context resets its canvass/session/health stores and purges its own QueryClient's `CASE_DATA_KEY_FAMILIES` entries — a cached list inside `staleTime` would otherwise render operator A's rows to operator B (#117). **All three per-context singletons are seeded**: canvass-store (view-context), session-store (`session-locked`/`unlocked` → `locked`/`active`, keeping the reused components' lock behavior in step with main), and health — which is **refresh-passive**: the main `useConnectionHealth` wake-refresh path is never mounted here (`auth.refreshSession()` throws under the accessToken proxy); the secondary's health display feeds from `subscribeToCaseActivity`'s `onStatus` + query results only. Reuses the same view components — pop-out is hosting, not forking — precisely because the client seam and store seeding make the context indistinguishable to them |
...
`src-tauri/crates/secure-vault/{Cargo.toml, src/lib.rs}` · `src-tauri/src/features/cloud_session/{mod.rs, types/mod.rs, services/mod.rs, commands/mod.rs}` · `src/lib/supabase/{client.ts, vault-storage.ts}` · `src/store/health-store.ts` · `src/hooks/useConnectionHealth.ts` · `src/features/cloud-session/{index.ts, types/index.ts, store/session-store.ts, hooks/{useAuthBootstrap,useIdleLock}.ts, services/{configService,authService}.ts, components/{SetupScreen,SignInScreen,SchemaGateScreen,LockOverlay,ConnectionIndicator}.tsx}` · `src/features/canvass/{index.ts, types/index.ts, store/canvass-store.ts, services/{canvassService,realtimeService,mediaService,mappers,geo,mapData,attention}.ts, hooks/{useCases,useCaseLocations,useCaseMedia,useLocationCounts,useCaseRealtime,useMediaPolling,useSignedUrl,useFlyTo}.ts, components/{CanvassRoot,NavRail,CasesView,MapCanvas,MarkerLayer,LocationCardStack,LocationCard,MediaThumb,VideoPlayer,DashboardView,ActivityFeed,SecondaryRoot}.tsx}` · `docs/developer/supabase-integration.md` · **A1/M7:** `src-tauri/src/features/view_windows/{mod,commands/mod,services/mod,types/mod}.rs` · `src-tauri/capabilities/view-windows.json` · `window.html` · `src/window-main.tsx` · `src/lib/supabase/secondary-client.ts` · `src/lib/services/sessionEvents.ts` · ~~`src/features/cloud-session/{services/diagnosticsService.ts, hooks/useDiagnostics.ts, components/DiagnosticsView.tsx}`~~ (A2: replaced by the panel) · **A2:** `src/features/process-panel/` (the adapted `processTerminal` **retained surface** — see 6.3A — + `sources/canvasHubSource.tsx` + diagnostics service/hook) · `src/features/canvass/components/ImageViewer.tsx` · `src/assets/fonts/` (4 woff2 families) — **≈ 65+ new files** (the exact count is generated by the tree, not this list — it has drifted three times; the enumeration above is the authority for *what*, not *how many*), all behind feature barrels, `lib/supabase`, and the global store/hooks layer (AD11).
...
| `src-tauri/src/features/mod.rs`, `src-tauri/src/bindings.rs` | 1.2 (`bindings.rs` also 6.3 — regenerated for the two moved commands; `features/mod.rs` needs no 6.3 touch, `cloud_session` registers at 1.2B) |
| `src-tauri/crates/platform-utils/src/lib.rs`        | 6.3 (A2 fix round — `tail_log`, 6.3B′) |
| `src/test/setup.ts`                                 | 1.2, 6.3       |
| `src/components/layout/MainWindow.tsx`              | 1.4 (3.2E's AD15 transform moved to `CanvassRoot` — fix-delta 2) |
| `src/components/layout/MainWindowContent.tsx`       | 1.4, 2.4       |
| `src/theme-variables.css`                           | 3.1 (3.1C — the template's `@theme` home) |
| `locales/{en,fr,ar}.json`                           | 1.4, 2.4, 4.3, 5.3, 6.3 |
...
| `docs/developer/README.md`                          | 6.4 (the docs pass — renumbered from 6.3 by A2) |
...
**Honesty metric:** 17 integration-point rows (≈22 physical files once multi-file rows — locales ×3, mod.rs+bindings.rs, types+service — are expanded; the two A1 rows touch one template file (`tauri.conf.json`, already counted) plus `vite.config.ts` and two plan-created files; plus regenerated `src/lib/bindings.ts` and lockfiles) across the Appendix A enumeration (its file count is deliberately uncounted — see Appendix A) — everything substantive is additive behind barrels. Plan-created files that later phases modify (`CanvassRoot`, `DashboardView`, `ActivityFeed`, `canvass/index.ts`) are tracked in their phase rows, not here — this table is the template-file blast radius.
...
**Total: 140** (9 Rust — 6 in `secure-vault`, 3 in `platform-utils` (A2 fix round, 6.3B′) — and 131 TypeScript; Revisions: #107 → 2.2, #108 → 6.1, #109 → 1.2, #110–121 → R4/A1 (with **#120 reassigned to Phase 6.3 by A2** — the panel owns diagnostics content), #122–126 → R5/A2: four ProcessPanel tests + the ImageViewer test, #127–129 → R5/A2 fix round: the `tail_log` Rust tests, #130–131 → R6/M3: the ledger-D17 realtime guard arms (Phase 2.3), #132–137 → R7/PR #6 review: marker-factory tests + the RTL padding pin (Phase 3.3) and the token-rejection pins (Phase 3.2), #138–140 → R8/M7: main-side session propagation (Phase 7.2 — session-rotation push (#138 flipped by the PR #10 review to the full TOKEN_REFRESHED/SIGNED_IN/USER_UPDATED class — H1), handshake reply, lock/sign-out broadcasts). **Ported 6.3A test files are excluded — the count is canvas-hub-authored tests only.**) Must reconcile with the Test Spec's closing summary; if counts drift during implementation, reconcile both docs before proceeding.
