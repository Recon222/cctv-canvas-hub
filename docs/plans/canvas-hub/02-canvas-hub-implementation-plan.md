...
| 4.3C (fix-delta 2) | `locales/{en,fr,ar}.json` | MODIFY | `canvass.imageViewer.*` keys — user-facing wall copy ships with its component's milestone, not two milestones later |
...
| 6.3D | `locales/{en,fr,ar}.json` | MODIFY | `processPanel.*` keys (lane labels ACTIVITY/SYSTEM, the collapsed SYS tab, inline error rows — ported keys re-namespaced under `processPanel.*`; `ar.json` is RTL). (`canvass.imageViewer.*` moved to 4.3C — fix-delta 2: keys ship with their component's milestone) |
