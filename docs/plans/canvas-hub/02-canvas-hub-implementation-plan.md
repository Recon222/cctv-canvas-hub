...
| ID   | File                             | Tag | Signatures                                                                                                                       |
| ---- | -------------------------------- | --- | --------------------------------------------------------------------------------------------------------------------------------- |
| 2.5A | `src/store/health-store.ts`      | NEW | `useHealthStore` (devtools, selector-only) · actions `recordEvent()` · `recordFetchOk()` · `recordFetchError()` · `channelStatus(s: ChannelStatus)` · exported pure `evaluate(marks, now): HealthState` · **canonical type definitions**: `HealthState = 'connecting' \| 'live' \| 'reconnecting' \| 'stale' \| 'offline'` and `ChannelStatus` (mapped from supabase-js subscribe states) — every other module imports these, never re-declares · constants `STALE_AFTER_MS = 90_000`, `RECONCILE_MS = 60_000` (**A2 as-built** — lowered from 5 min by the PR #4 cadence finding; **invariant: `RECONCILE_MS < STALE_AFTER_MS`, test-pinned** by the health-store tripwire), `SIGNED_URL_KEY_PREFIX = 'signed-url'` · the case-data family constants (`CASES_KEY`/`LOCATIONS_KEY`/`LOCATION_COUNTS_KEY`/`MEDIA_KEY`), the `CASE_DATA_KEY_FAMILIES` tuple, and `isCaseDataKey()` — the catch-up **allow-list** registry (AD11): every case-data family registers here or gets no catch-up |
