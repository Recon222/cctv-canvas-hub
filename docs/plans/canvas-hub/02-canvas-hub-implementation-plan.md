...
| ID   | File                                                       | Tag    | Signatures                                                                                                       |
| ---- | ----------------------------------------------------------- | ------ | ------------------------------------------------------------------------------------------------------------------ |
| 7.1A | `src-tauri/src/features/view_windows/{mod,commands/mod,services/mod,types/mod}.rs` | NEW | `pub async fn open_view_window(app: AppHandle, view: ViewWindow, case_id: Option<String>) -> Result<(), String>` — `ViewWindow` is a four-derive `#[serde(rename_all = "lowercase")] enum { Case, Map, Diagnostics }` (invalid views unrepresentable over IPC; deliberately distinct from the store's view union, which also has `'cases'`). Focus-if-open **re-emits `view-context`** so re-popping for a different case retargets the existing window; create-once otherwise (`window.html?view=…`). **Double-open race arm:** on build-Err, re-check for the window — if it now exists, focus + return Ok; destroy only on genuine partial creation (two interleaved async opens must not destroy the winner) · `pub async fn close_view_window(app, view: ViewWindow)` · `pub async fn read_log_tail(app, lines: u32) -> Result<String, String>` — `lines` clamped (≤ 500), **seek-from-end bounded read (≤ 64 KB), never whole-file** (an uncapped kiosk log grows for weeks); path pinned: `app.path().app_log_dir()?/tauri-app.log`; content is token-free by T3, redact defensively anyway · `pub async fn vault_status(app) -> Result<VaultStatus, String>` — presence-only booleans + mtimes (`config_present`, `vault_present`, `keyring_key_present`) — NEVER key or session material (`vault_get` is the wrong tool: it returns the decrypted secret) |
| 7.1B | `window.html` + `src/window-main.tsx`                       | NEW    | secondary entry point (vite input, `quick-pane.html` precedent): **branches on `view`** — `diagnostics` → `DiagnosticsView` (cloud-session barrel), else → `SecondaryRoot` (canvass barrel); both components barrel-exported — no deep imports (aliased fails `ast:lint`; relative is the forbidden App.tsx idiom, AD11) |
...
| ID   | File                                                     | Tag    | Signatures                                                                                                     |
| ---- | --------------------------------------------------------- | ------ | ------------------------------------------------------------------------------------------------------------------ |
| 7.3A | `src/features/canvass/components/SecondaryRoot.tsx`      | NEW    | hosts `MapCanvas`-view or `DashboardView` with its own QueryClient + realtime subscription. Boot order pinned: `secondary-ready` handshake → token installed (`initSecondaryClient`, which claims this context's `getSupabase()` seam) → `view-context` seeds **this context's own** canvass-store (`selectCase(caseId)`, `setView`) → queries + `subscribeToCaseActivity(caseId, …)` mount. Reuses the same view components — pop-out is hosting, not forking — precisely because the client seam and store seeding make the context indistinguishable to them |
...
| 7.3C | `src/features/cloud-session/{services/diagnosticsService.ts, hooks/useDiagnostics.ts, components/DiagnosticsView.tsx}` | NEW  | services-own-IPC: `diagnosticsService` wraps `commands.readLogTail` + `commands.vaultStatus` (a component calling `commands.*` directly would slip the ast-grep gate — it only matches `invoke()` — but still violates the rule); `useDiagnostics` polls on a slow interval; `DiagnosticsView` renders health-state detail (current + recent transitions), log tail, presence-only vault/keyring status, app + `APP_REQUIRED_SCHEMA_VERSION` |
