...
| 1.2C | `src/lib/supabase/vault-storage.ts`                    | NEW    | `export const vaultStorage: { getItem(k): Promise<string \| null>; setItem(k, v): Promise<void>; removeItem(k): Promise<void> }` → wraps `commands.vaultGet/Set/Clear` |
...
**Goal:** the health state machine (doc 01 §5.4) fed by realtime status + fetch results, homed in the global layer (AD11).

| ID   | File                             | Tag | Signatures                                                                                                                       |
| ---- | -------------------------------- | --- | --------------------------------------------------------------------------------------------------------------------------------- |
| 2.5A | `src/store/health-store.ts`      | NEW | `useHealthStore` (devtools, selector-only) · actions `recordEvent()` · `recordFetchOk()` · `recordFetchError()` · `channelStatus(s)` · exported pure `evaluate(marks, now): HealthState` · constants `STALE_AFTER_MS = 90_000` |
| 2.5B | `src/hooks/useConnectionHealth.ts` | NEW | wires listeners (`online`/`offline`/`visibilitychange`) + interval `evaluate` → health-store; reconnect ⇒ `queryClient.invalidateQueries()` — unfiltered catch-up, no feature key knowledge (Flow E, AD11) |
...
| ID   | File                                                     | Tag    | Signatures                                                                                                     |
| ---- | -------------------------------------------------------- | ------ | --------------------------------------------------------------------------------------------------------------- |
| 6.2A | `src/hooks/useConnectionHealth.ts` + `src/lib/supabase/client.ts` | MODIFY | on `visible`/`online`/resubscribe: `auth.refreshSession()` → `realtime.setAuth()` → `queryClient.invalidateQueries()` (Flow E3, AD11) |
...
**Total: 106** (6 Rust in `secure-vault`, 100 TypeScript). Must reconcile with the Test Spec's closing summary; if counts drift during implementation, reconcile both docs before proceeding.
