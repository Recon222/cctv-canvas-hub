...
| AD15 (A2) | Scale-to-fit shell strategy | The design canvas is 1920×1080 with `transform: scale(vw/1920)` on the shell. As-built: **scale the chrome only; the Mapbox canvas stays unscaled-native** (map fills its container at real pixels; chrome overlays scale) — final mechanics decided at Phase 3.2 with a live check. **The transform's owning file is a named phase row: `MainWindow.tsx` (3.2E)** — the chrome wrapper carries the scale; the Mapbox subtree stays outside the scaled ancestor | _Whole-shell scaling rejected pending proof_: Mapbox GL inside a CSS-transformed ancestor has known pointer-math and tile-crispness pitfalls; if the M3 live check shows the simple whole-shell scale works flawlessly on the target display, the simpler option may win — the decision point is pinned, not the outcome. |
...
| 3.2E (A2 fix round) | `src/components/layout/MainWindow.tsx` | MODIFY | carries the **AD15 shell transform**: `transform: scale(vw/1920)` on the chrome wrapper only — the Mapbox canvas subtree stays outside the scaled ancestor (unscaled-native); final mechanics live-checked here per AD15 (the decision point is pinned, not the outcome) |
...
| ID   | File                                                       | Tag    | Signatures                                                                                                     |
| ---- | ----------------------------------------------------------- | ------ | ------------------------------------------------------------------------------------------------------------------ |
| 6.3A | `src/features/process-panel/` (adapted `processTerminal` surface + adapter) | NEW | **Retained surface** (the pure lane renderers + their supporting services): `TextLane.tsx`, `RichLane.tsx` + the `rich/*` card components (`ArtifactCard`, `FenceCard`, `KvGrid`, `LogLine`, `ProgressBar`, `SectionMarker`, `StatusLine` — minus `TableCard`), `ansiParser.tsx`/`ansiSgr.ts`/`parseAnsiToHtml.ts`/`vtEngine.ts`, `crt.css`, `Header.tsx`/`Footer.tsx` chrome, `EmptyPanel.tsx`. **Discarded:** the `ProcessTerminal` dispatcher, `SessionPanel` and its `SessionData`/`PipelineMessage` contract, `sources/ProcessTerminal{SDK,Claw}.tsx`, the agent-pipeline feed model (`feedReducer`/`coalescer`/`displayData`/`displayPayloads`), `TableCard` (and with it the `react-data-grid` dependency — AD14: decided, dropped), and `BlockFeed`/`CallBlockView`/`NoteRowView` unless trivially generic. **Adapter pinned as a signature:** `sources/canvasHubSource.tsx` producing `ProcessPanelRow { at: number; lane: 'activity' \| 'system'; source: string; text: string; tone?: 'info' \| 'warn' \| 'error' }`, fed by (a) a vanilla `useHealthStore.subscribe` diff (~5 lines — the store keeps **no transition log** and is **not** created with `subscribeWithSelector`, so the adapter diffs `state` across notifications), (b) `readLogTail` polling via a diagnostics service wrapping `commands.readLogTail`/`commands.vaultStatus`, and (c) the canvass activity ring via the read-only selector the canvass barrel exports for exactly this (6.3E — the AD11 seam). **Poll discipline:** `LOG_TAIL_POLL_MS = 5_000`; the poll runs only while *expanded AND the SYSTEM lane is active* (a kiosk session is measured in days — no 64 KB disk reads for a lane nobody is looking at); a tick is skipped while a read is in flight (an unguarded interval stacks concurrent reads). The ACTIVITY lane **hosts `ActivityFeed.tsx`** (component reuse, relocated from the dashboard by 6.3C — not the ported row renderer). **Ported tests:** only files covering the retained surface come across (`ansiParser`, `vtEngine`/`TextLane`, `rich`), adapted to compile; dispatcher/source tests are dropped with their subjects. Ported tests run in the gate but are **excluded from the numbered count** — doc 03 counts canvas-hub-authored tests only. Gate-passed: strictTypeChecked, ast-grep, i18n keys (6.3D), selector-only stores |
...
| 6.3B′ | `src-tauri/crates/platform-utils/src/lib.rs` | MODIFY | pure free function `pub fn tail_log(bytes: &[u8], max_lines: usize) -> String` — `String::from_utf8_lossy` (a seek-from-end read starts at an arbitrary byte offset and can land mid-codepoint; the log is not ASCII-only — `TargetKind::Webview` routes frontend `fr`/`ar` strings into the same file), **drop the first (partial) line** whenever the slice is mid-file, clamp to the last `max_lines`. Inline `#[cfg(test)]`: 3 tests (#127–129 — mid-codepoint split; clamp above the window; input shorter than the window). The Tauri-free workspace crate is the one place this logic is unit-testable on Windows (`[lib] test = false` in the app crate) |
| 6.3C | `CanvassRoot.tsx` + `DashboardView.tsx` + `src/store/ui-store.ts` + panel chrome | MODIFY | **Mount site: inside `CanvassRoot`'s flex row** (the board tree — gated on `active`/`locked` like everything there, so the SYSTEM poll and ACTIVITY lane never run during `booting`/`needs-setup`/`signed-out`/`schema-gate` on an unattended board, and the panel's lifecycle aligns with the ring/store resets on unmount). There is no "RightSideBar slot" — neither template sidebar renders anywhere (A1's NavRail precedent is likewise a bespoke board-tree component); the panel occupies the **right-edge chrome position** as owner-superseded chrome (AD9). **Expanded/collapsed state IS `useUIStore.rightSidebarVisible`** (default flipped to open here) — the template's existing shortcut and show/hide-right-sidebar palette commands drive the panel with zero new plumbing; only **lane selection** is new panel-local state. **Posture:** default-open on ACTIVITY on the `cases`/`case` views (wall posture — spec §6); on the `map` view the panel **auto-collapses to the SYS tab**, and when expanded there it **overlays** the card stack — map and stack never reflow (no width coupling, no `map.resize()` trigger — 3.2D). Collapsed = slim SYS tab; header toggle per the design. **Feed relocation:** `ActivityFeed` moves out of `DashboardView` into the ACTIVITY lane; the dashboard recomposes — the roster takes the freed column (test #98 updated in place to assert the recomposition) |
| 6.3D | `locales/{en,fr,ar}.json` | MODIFY | `processPanel.*` keys (lane labels ACTIVITY/SYSTEM, the collapsed SYS tab, inline error rows — ported keys re-namespaced under `processPanel.*`; `ar.json` is RTL) + `canvass.imageViewer.*` keys (4.3A′'s `PHOTO n OF N` header) |
| 6.3E | `src/features/canvass/index.ts` | MODIFY | barrel-export a **read-only activity selector/hook** (e.g. `useActivityRing(): readonly ActivityEntry[]`) for the panel — the AD11 `process-panel → canvass` seam; a deep import into `canvass/store/canvass-store` fails `ast:lint` (`barrel-export-enforcement.yml`, severity error) |
...
| `src/components/layout/MainWindow.tsx`              | 1.4, 3.2 (AD15 shell transform — 3.2E) |
| `src/components/layout/MainWindowContent.tsx`       | 1.4, 2.4       |
| `src/theme-variables.css`                           | 3.1 (3.1C — the template's `@theme` home) |
| `src/store/ui-store.ts`                             | 6.3 (`rightSidebarVisible` default flips open — 6.3C) |
| `locales/{en,fr,ar}.json`                           | 1.4, 2.4, 5.3, 6.3 |
