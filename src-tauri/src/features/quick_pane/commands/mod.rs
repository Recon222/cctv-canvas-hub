...
fn is_quick_pane_visible(app: &AppHandle) -> bool {
    #[cfg(target_os = "macos")]
    {
        // panel.is_visible() is a raw AppKit message send — it must run on the
        // main thread, but callers (toggle_quick_pane) are on the async runtime.
        let app_for_closure = app.clone();
        read_on_main_thread(app, move || {
            app_for_closure
...
/// Runs a fallible closure on the main thread and waits for its result.
///
/// `show_quick_pane` / `dismiss_quick_pane` are `async` so they don't occupy the
/// main thread / WebView2 event loop on Windows (see AGENTS.md "Window-Creating
/// Commands Must Be `async`"), which means they run on the async runtime — NOT
/// the main thread. macOS NSPanel/AppKit operations must run on the main thread,
/// so this hops back to it and propagates the closure's result.
#[cfg(target_os = "macos")]
fn run_on_main_thread<F>(app: &AppHandle, f: F) -> Result<(), String>
where
    F: FnOnce() -> Result<(), String> + Send + 'static,
{
    ...
    rx.recv()
        .map_err(|e| format!("Main-thread task was dropped: {e}"))?
}

/// Reads a value on the main thread and returns it (None on dispatch/recv
/// failure). For AppKit reads (e.g. NSPanel `is_visible`) that must run on the
/// main thread even though the caller is on the async runtime.
#[cfg(target_os = "macos")]
fn read_on_main_thread<T, F>(app: &AppHandle, f: F) -> Option<T>
where
    T: Send + 'static,
...
#[tauri::command]
#[specta::specta]
pub async fn show_quick_pane(app: AppHandle) -> Result<(), String> {
...
#[tauri::command]
#[specta::specta]
pub async fn dismiss_quick_pane(app: AppHandle) -> Result<(), String> {
    #[cfg(target_os = "macos")]
    {
        let app_for_closure = app.clone();
        run_on_main_thread(&app, move || {
            if let Ok(panel) = app_for_closure.get_webview_panel(QUICK_PANE_LABEL) {
...
#[tauri::command]
#[specta::specta]
pub async fn toggle_quick_pane(app: AppHandle) -> Result<(), String> {
    log::info!("Toggling quick pane window");

    if is_quick_pane_visible(&app) {
        dismiss_quick_pane(app).await
    } else {
    ...
    global_shortcut
        .on_shortcut(shortcut, move |_app, _shortcut, event| {
            ...
            if event.state == ShortcutState::Pressed {
                ...
                // toggle_quick_pane is async now; drive it on the async runtime
                let app_handle = app_handle.clone();
                tauri::async_runtime::spawn(async move {
                    if let Err(e) = toggle_quick_pane(app_handle).await {
