...
use crate::features::recovery::types::{validate_filename, RecoveryError, MAX_RECOVERY_DATA_BYTES};
...
pub async fn load_emergency_data(app: AppHandle, filename: String) -> Result<Value, RecoveryError> {
    ...
    // Guard against oversized files (mirrors the write-side 10MB cap) so a
    // tampered or externally-created file can't be read unbounded into memory.
    let file_size = std::fs::metadata(&file_path)
        .map_err(|e| RecoveryError::IoError {
            message: e.to_string(),
        })?
        .len();
    if file_size > MAX_RECOVERY_DATA_BYTES as u64 {
        return Err(RecoveryError::DataTooLarge {
            max_bytes: MAX_RECOVERY_DATA_BYTES,
        });
    }
